...
class AnalysisTask(Base):
    ...
    avg_execution_time = Column(String(20), default="0.0", comment="平均执行时间（秒）")
    ...
    def update_execution_stats(self, success: bool, execution_time: float = 0.0, tokens_used: int = 0, cost: float = 0.0):
        ...
        if execution_time > 0:
            try:
                ...
                self.avg_execution_time = f"{new_avg:.3f}"
            except (ValueError, TypeError):
                self.avg_execution_time = f"{execution_time:.3f}"
    ...
    def can_execute(self) -> bool:
        ...
        return (
            ...
            self.ai_model_id is not None and
            self.storage_credential_id is not None
        )
    ...
    def validate_config(self) -> dict:
        ...
        if not self.storage_credential_id:
            errors.append("需要指定存储凭证")

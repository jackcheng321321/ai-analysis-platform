...
class StorageCredential(Base):
    ...
    # 认证信息（加密存储）
    username_encrypted = Column(Text, comment="加密的用户名")
    password_encrypted = Column(Text, comment="加密的密码")
    access_key_encrypted = Column(Text, comment="加密的访问密钥")
    secret_key_encrypted = Column(Text, comment="加密的密钥")
    token_encrypted = Column(Text, comment="加密的令牌")
    ...
    def to_dict(self, include_sensitive=False):
        ...
        if include_sensitive:
            ...
            data.update({
                "has_username": bool(self.username_encrypted),
                "has_password": bool(self.password_encrypted),
                "has_access_key": bool(self.access_key_encrypted),
                "has_secret_key": bool(self.secret_key_encrypted),

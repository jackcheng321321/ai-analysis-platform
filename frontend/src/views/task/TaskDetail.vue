  ...
  <div class="task-detail">
    ...
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <el-button :icon="VideoPlay" @click="executeTask" :loading="executing" v-if="task.is_active">
          执行任务
        </el-button>
        ...
        <el-col :xs="24" :lg="12">
          <el-card class="info-card" shadow="never">
            ...
            <div class="config-section">
              <h4>AI模型配置</h4>
              <div v-if="task.ai_model_id">
                <div class="info-item">
                  <label>模型ID:</label>
                  <span>{{ task.ai_model_id }}</span>
                </div>
              </div>
            ...
            <div class="config-section">
              <h4>存储配置</h4>
              <div v-if="task.storage_credential_id">
                <div class="info-item">
                  <label>存储凭证ID:</label>
                  <span>{{ task.storage_credential_id }}</span>
                </div>
              </div>
            ...
            <div class="config-section">
              <h4>Webhook配置</h4>
              <div v-if="task.webhook_id">
                <div class="info-item">
                  <label>Webhook ID:</label>
                  <span>{{ task.webhook_id }}</span>
                </div>
              </div>
        ...
        <el-col :xs="24" :lg="12">
          <el-card class="schedule-card" shadow="never">
            ...
            <div class="config-section">
              <div class="info-item">
                <label>定时执行:</label>
                <el-tag :type="task.is_scheduled ? 'success' : 'info'" size="small">
                  {{ task.is_scheduled ? '是' : '否' }}
                </el-tag>
              </div>
              
              <div v-if="task.is_scheduled && task.cron_expression" class="info-item">
                <label>Cron表达式:</label>
...
const executeTask = async () => {
  ...
  try {
    await taskApi.execute(task.value.id)
